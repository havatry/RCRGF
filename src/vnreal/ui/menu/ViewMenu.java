...
public final class ViewMenu extends JMenu implements ActionListener {
	public ViewMenu() {
		...
		add(UI.getInstance().getBottomPane().getHiddenTabs());
	}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (cmd.equals("node_labels")) {
			NodeLabelTransformer<?> nodeLT = UI.getInstance().getGraphPanel()
					.getNodeLT();
			nodeLT.setEnabled(!nodeLT.isEnabled());
			UI.getInstance().getGraphPanel().updateUI();
		} else if (cmd.equals("link_labels")) {
			LinkLabelTransformer<?> linkLT = UI.getInstance().getGraphPanel()
					.getLinkLT();
