...
public class ConstraintsGeneratorDialog extends AbstractButtonDialog {
	...
	public ConstraintsGeneratorDialog() {
		super(UI.getInstance(), "Generate Constraints", "Generate",
		...
		try {
			...
			if (resClass.getSimpleName().equals("IdResource")) {
				...
				for (int p = 0; p < paramNames.length; p++) {
					...
					setter.invoke(res, Math.rint(Double.parseDouble(maxValues[p]) * stream.nextDouble()));
				}
		...
		try {
			if (demClass.getSimpleName().equals("IdDemand")) {
				...
				for (int p = 0; p < paramNames.length; p++) {
					...
					setter.invoke(dem, Math.rint(Double.parseDouble(maxValues[p]) * stream.nextDouble()));
				}
