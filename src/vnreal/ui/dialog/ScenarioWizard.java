...
public class ScenarioWizard extends AbstractButtonDialog {
	...
	public ScenarioWizard(Scenario scenario) {
		super(UI.getInstance(), "Scenario Wizard", "Create", new Dimension(
				400, 300));
	...
	protected void doAction() {
		...
		UI.getInstance().getGraphPanel().autoZoomToFit();
	}
	...
	public static NetworkStack generateTopology(int snNodes, double snAlpha,
		...
		NetworkStack result;
		result = new NetworkStack(new SubstrateNetwork(false, Constants.DIRECTED), // �����
...
//		}
		...
		for (int layer = 1; layer <= vnNumber; layer++) {
			if (Constants.DIRECTED) {
				...
				vn = new VirtualNetwork(layer, true, false); // �������������
			}
			result.addLayer(vn);
