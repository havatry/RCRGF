...
import mulavito.ui.control.AbstractPopupMousePlugin;
...
import vnreal.ui.UI;
import vnreal.ui.MyGraphPanel;
import vnreal.ui.dialog.AddConstraintDialog;
import vnreal.ui.dialog.EditConstraintDialog;
import vnreal.ui.dialog.RemoveConstraintDialog;
	...
	protected void createGeneralMenuEntries(final Point point, final N graph,
		...
		mi.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				while (addConstraint) {
					new AddConstraintDialog((NetworkEntity<AbstractConstraint>) node, layer, UI.getInstance(),
		...
		@SuppressWarnings("rawtypes")
		final MyGraphPanel pnl = UI.getInstance().getGraphPanel();
...
//		popup.add(mi);
	...
	private void addLink(int layer, V src, V dest) {
		...
		while (addConstraint) {
			new AddConstraintDialog((NetworkEntity<AbstractConstraint>) edge, layer, UI.getInstance(),
					new Dimension(300, 150));
			// if a resource/demand has been added
			if (edge.get().size() > 0) {
				addConstraint = false;
	...
	private void createAddConstraintMenuItem(
		...
		mi.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				new AddConstraintDialog(ne, layer, UI.getInstance(),
						new Dimension(300, 150));
			}
		});
		popup.add(mi);
	...
	private void createEditConstraintMenuItem(
		...
		mi.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				new EditConstraintDialog(ne, layer, UI.getInstance(),
						new Dimension(300, 150));
			}
		});
		popup.add(mi);
	...
	private void createRemoveConstraintMenuItem(
		...
		mi.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				new RemoveConstraintDialog(ne, layer, UI.getInstance(),
						new Dimension(300, 150));
			}
		});
		popup.add(mi);
